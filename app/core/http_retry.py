...
def _is_transient_error(exc: BaseException) -> bool:
    """Only network-level failures and 5xx responses are retried. A 4xx means
    the request itself was rejected (bad payload, not found, ...); retrying it
...
async def post_with_retry(client: httpx.AsyncClient, url: str, json: dict) -> httpx.Response:
    """POSTs with retry-with-backoff (up to 3 attempts) on connection errors,
